...
void slNRF24_Config() {
    ...
    CSN_LOW(); slSPI_TransferInt(W_REGISTER | SETUP_RETR); slSPI_TransferInt(0x1f); CSN_HIGH();//wait 500uS up to 15 retransmits
    ...
    CSN_LOW(); slSPI_TransferInt(W_REGISTER | SETUP_AW); slSPI_TransferInt(0x03); CSN_HIGH();//Setup of Address Widths
...
void slNRF24_PowerUp(){
    CSN_LOW(); slSPI_TransferInt(W_REGISTER | CONFIG); slSPI_TransferInt(1 << PWR_UP | 1 << EN_CRC ); CSN_HIGH();
    //CSN_LOW(); slSPI_TransferInt(W_REGISTER | CONFIG); slSPI_TransferInt(1 << PWR_UP); CSN_HIGH();
}

void slNRF24_PowerDown(){
    CE_LOW();
    CSN_LOW(); slSPI_TransferInt(W_REGISTER | CONFIG); slSPI_TransferInt(1 << EN_CRC); CSN_HIGH();
